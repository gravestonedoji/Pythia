...
- **Daily forecast (the brain):** `Codex-opus-4-8`. Calibration is the whole
  point and it runs once per ticker per day, so cost is trivial. Do not downgrade.
- **Weekly self-review (v1+):** `Codex-opus-4-8`.
- **Data/news triage, extraction, tagging (v1+):** `Codex-haiku-4-5`.
